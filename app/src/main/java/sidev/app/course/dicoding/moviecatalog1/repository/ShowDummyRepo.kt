...
object ShowDummyRepo: ShowRepo {
    override suspend fun getPopularMovieList(c: Context?): Result<List<Show>> = Success(listOf(AppConfig.dummyShowItem))
    override suspend fun getPopularTvList(c: Context?): Result<List<Show>> = Success(listOf(AppConfig.dummyShowItem))
